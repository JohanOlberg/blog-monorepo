...
export class ChangeAuthorUserController {
    changeAuthorUserUseCase;
    constructor(changeAuthorUserUseCase) {
        this.changeAuthorUserUseCase = changeAuthorUserUseCase;
    }
    async handle(request, reply) {
        const params = authorIdParamSchema.parse(request.params);
        const body = changeAuthorUserSchema.parse(request.body);
        const author = await this.changeAuthorUserUseCase.execute(params.id, body.userId);
        return reply.status(200).send(author);
    }
}
//# sourceMappingURL=ChangeAuthorUserController.js.map